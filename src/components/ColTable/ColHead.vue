...
export default {
    ...
    render(h) {
        ...
        return <table class="editTable__block col" id="tb"><thead>{tr}</thead></table>
    },
    props: {
        allRow: {
            type: Boolean,
    ...
    methods: {
      ...
      createHead() {
        ...
        // //左下角部分；
        if(this.allRow) {
            window.console.log("只有一个纵向表头");
            this.edit.resetNewColHead.forEach((x) => {
                let cell = [];
                x.forEach((y) => {
                    if(y && y !== null) {
                        let height = y.height ? y.height: y.colSpan * this.edit.tableTdHeight + "px";
                        cell.push(<th colspan={y.rowSpan} rowspan={y.colSpan} style={{ width: y.width  ,height: height, textAlign: this.edit.textAlign}}>{y.name}</th>)
                    }
                })
                tr.push(<tr>{cell}</tr>);
            });
